...
test_that("min value raises error if incorrect format", {
    ...
    expect_error( new_validator(
        ...
        data = df
    ), "Invalid date format for min_date in column 'date': 01-01-2012, use Year Month Day format")
})
...
test_that("max value raises error if incorrect format", {
    ...
    expect_error( new_validator(
        schema = list(columns = columns, check_duplicates = FALSE, check_completeness = FALSE),
        data = df
    ), "Invalid date format for max_date in column 'date': 30/06/2024, use Year Month Day format")
})
...
test_that("max and min value no error for different formats", {
    columns <- list(
        ...
        end_date = list(type = "date", optional = TRUE, min_date = 20260101, max_date = "2025-12-31")
    )
